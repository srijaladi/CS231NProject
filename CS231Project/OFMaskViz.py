...
def gen_batch_pure(data, flow_data, frame_diff: int):
    l = int(len(data))
    b_idxs = torch.arange(l - frame_diff - frame_diff - frame_diff).long()
    a_idxs = b_idxs + frame_diff + frame_diff
    t_idxs = b_idxs + frame_diff
...
def save_image_pure(filepath, image):
    np_img = conv_image(image)
    plt.clf()
...
def save_examples_mod(mode, model, batch):
    assert mode == "val" or mode == "test" or mode == "train" or mode == "train_val"
    loss_per = model.eval_loss_per(*batch)
    ...
    for i in range(1,6):
        idx = idxs[-i]
        x_1, x_2, x_3, t_x = batch[0][idx], batch[1][idx], batch[2][idx], batch[3][idx]
        out_x = model(x_1, x_2, x_3)
        folderpath = EXAMPLES_FOLDER + mode + "_worst_" + str(i)
        mask_x = model.maskModel(x_3)[0]
        mask_x = mask_x.float() - 0.0001
        fin_mask_x = torch.cat((mask_x, mask_x, mask_x), axis = 0)
        fp = EXAMPLES_FOLDER + mode + "_worst_" + str(i) + "_mask.png"
        save_images(folderpath, (x_1, x_2, t_x), out_x)
        print(fin_mask_x.size())
        print(mask_x)
        save_image_pure(fp, fin_mask_x)
...
"""
f_l = [torch.nn.Conv2d(8, 64, k_size, padding = 'same'), torch.nn.ReLU(), 
       torch.nn.Conv2d(64, 32, k_size, padding = 'same'), torch.nn.MaxPool2d((2,2), stride = 2), torch.nn.ReLU(), 
...
m_l = [torch.nn.Conv2d(2, 3, k_size, padding = 'same'), torch.nn.ReLU(), torch.nn.Conv2d(3, 2, k_size, padding = 'same'), torch.nn.ReLU(), torch.nn.Conv2d(2, 1, k_size, padding = 'same'), torch.nn.Sigmoid()]

learning_rates = {'features':1e-3, 'generation':1e-3}

FrameModel = AdvancedFlowFrameGenModel(in_h, in_w, f_l, g_l, m_l, torch.nn.functional.mse_loss, learning_rates)
FrameModel.load_state_dict(torch.load("DataFolder/OldLogs/OFMaskLargeModelSmallSize/model_params.txt", map_location='cpu'))
...
b1 = np.asarray(Image.open('b1.png'))
a1 = np.asarray(Image.open('a1.png'))

b1 = np.swapaxes(np.swapaxes(b1, 1, 2), 0, 1)
a1 = np.swapaxes(np.swapaxes(a1, 1, 2), 0, 1)

print(a1.shape, b1.shape)

of = optical_flow_pair(a1, b1)

print(a1.shape, b1.shape, of.shape)
"""
...
save_examples_mod("test", FrameModel, test_batch)
